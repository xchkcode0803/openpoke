...
Artifacts go under `.deepeval/campaigns/difficult-routing-v1/{capacity,live}/{variant}/`. A completed outcome is not rerun automatically. Use another explicit `EVAL_CAMPAIGN_DIR` for a separate campaign; do not use it to bypass an approved spending cap. Full model/tool traces are retained, while million-name rosters are represented by reconstruction metadata rather than repeated in turn results.
