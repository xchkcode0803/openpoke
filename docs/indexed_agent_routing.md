...
The paid comparison runs 99 original full cases, 84 stress variants, and six inspection cases once, sequentially, using the same $10 ledger across interaction and judges. It adapts fixture storage through the public bulk-import API; it does not alter fixture expectations. Completed outcomes are not automatically rerun. The runner pauses the whole campaign when usage is unsettled, the spending cap blocks a request, or a provider payment rejection occurs. An interrupted request remains reserved until reconciled; resuming with its full worst-case charge held requires explicit user approval. A documented HTTP 402 rejection is recorded as a pre-generation failure, not as a billed generation. Confirmed infrastructure corrections must be documented and limited to affected cases.

The [separate first-pass report](indexed_agent_routing_results.md) records behavioral changes, tool-error recoveries, cold-build cost, warm resource use, model charges, and operational interruptions. No tuning loop follows this first pass.
