...
from .emulator import DATE, USER
...
def mime_message(arguments: dict, *, subject: str | None = None, body: str | None = None,
    ...
    message["From"] = USER
    message["To"] = ", ".join([arguments["recipient_email"], *arguments.get("extra_recipients", [])])
    ...
    message.set_content(arguments.get("body", "") if body is None else body,
...
class GmailAdapter:
    ...
    def __call__(self, action, user_id, arguments=None):
        ...
        try:
            for fault in self.faults:
                ...
                if remaining and action == spec.action and (not spec.recipient or args.get("recipient_email") == spec.recipient):
                    fault[1] -= 1
            ...
            self.emit("gmail_action", action=action, arguments=args, result=result,
                      before=before, after=self.emulator.snapshot())
            return result
        except Exception as exc:
            if isinstance(exc, UnsupportedOperation):
                self.unsupported.append(str(exc))
            self.emit("gmail_action", action=action, arguments=args, error=str(exc),
                      before=before, after=self.emulator.snapshot())
            raise
    ...
    def execute(self, action, args):
        ...
        if action == "GMAIL_LIST_DRAFTS":
            ...
            if args.get("verbose"):
                result["drafts"] = [self.request("GET", "drafts/" + d["id"], params={"format": "full"})
        ...
        if action == "GMAIL_REPLY_TO_THREAD":
            thread = self.request("GET", "threads/" + quote(args["thread_id"], safe=""), params={"format": "full"})
            ...
            raw = mime_message(args, subject=subject, body=args["message_body"], reply_id=headers.get("message-id"))
            return self.request("POST", "messages/send", json={"raw": raw, "threadId": args["thread_id"]})
    ...
    def fetch_message(self, message_id):
        message = self.request("GET", "messages/" + quote(message_id, safe=""), params={"format": "full"})
        ...
        if payload.get("mimeType") == "text/html":
            message["htmlBody"] = base64.urlsafe_b64decode(payload.get("body", {}).get("data", "")).decode("utf-8", errors="replace")
        return {**message, "subject": headers.get("subject", ""), "sender": headers.get("from", ""),
                "to": headers.get("to", ""), "messageTimestamp": parsedate_to_datetime(headers.get("date", DATE)).isoformat()}
