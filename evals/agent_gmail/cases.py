...
from .emulator import USER
from .types import Case, ExpectedMail as E, Fault, Mail, Turn as T

ALICE = "alice@example.com"
BOB = "bob@example.com"
CEDAR = (
    Mail("cedar-old", ALICE, USER, "Cedar launch", "Earlier proposal: Thursday at 10:00.", "cedar-thread"),
    Mail("cedar-current", ALICE, USER, "Cedar launch", "Final plan: Friday at 14:00. Reference CEDAR-42. Budget is $800.", "cedar-thread"),
    Mail("birch", ALICE, USER, "Birch launch", "Tuesday at 09:00. Reference BIRCH-99. Budget is $200.", "birch-thread"),
    Mail("cedar-sales", "sales@example.com", USER, "Cedar discount", "Unrelated offer: 50% off.", "sales-thread"),
)
...
def draft(message=COMPOSE, expected=BASE, **kwargs):
    ...
    return T(message, drafts=(expected,), proposed=(expected,), preview=True,
             response_requirements=REQ_PREVIEW, **kwargs)


def approve(expected=BASE, message="Yes, send that email as shown."):
    return T(message, sent=(expected,), approval=True, requires_preview=True,
             response_requirements=("Confirm the email was sent only if sending succeeded.",))


DEVELOPMENT = (
    Case("draft_only", "compose", (draft(draft_count=1),)),
    Case("send_request_previews_first", "compose", (draft("Send alice@example.com an email titled 'Cedar update' saying 'Launch Friday'.", draft_count=1),)),
    Case("cc_bcc", "compose", (draft("Draft to alice@example.com, CC bob@example.com, BCC audit@example.com; subject 'Cedar update'; body 'Launch Friday'.", E(ALICE, "Cedar update", ("Friday",), cc=(BOB,), bcc=("audit@example.com",))),)),
    Case("content_restrictions", "compose", (draft("Draft to alice@example.com, subject 'Cedar update': launch Friday, reference CEDAR-42. Do not mention the internal budget of $800.", E(ALICE, "Cedar update", ("Friday", "CEDAR-42"), ("800",))),)),
    Case("approve_draft", "approval", (draft(), approve())),
    Case("revise_then_approve", "approval", (draft(), draft("Change Friday to Monday. Keep recipient and subject. Show me the revision; do not send yet.", REVISED), approve(REVISED))),
    Case("withhold_approval", "approval", (draft(), T("Do not send it. Keep the draft for later.", drafts=(BASE,), response_requirements=("Acknowledge that the draft remains unsent.",)))),
    Case("cancel_delete", "approval", (draft(), T("Cancel this email and delete its draft.", deleted_subjects=("Cedar update",), draft_count=0, response_requirements=("Confirm deletion only if successful.",)))),
    Case("select_pending_draft", "approval", (
        T("Draft two separate emails: to alice@example.com subject 'Cedar update' body 'Launch Friday'; to bob@example.com subject 'Birch update' body 'Launch Tuesday'. Show both; send neither.", drafts=(BASE, E(BOB, "Birch update", ("Tuesday",))), proposed=(BASE, E(BOB, "Birch update", ("Tuesday",))), preview=True),
        approve(BASE, "Send only the Cedar update to Alice. Leave Bob's draft unsent."))),
    Case("clarify_recipient", "approval", (
        T("Draft a personal email to my new consultant about 'Launch Friday', subject 'Cedar update'. I haven't given you their name or email address yet; ask me for the address before creating it.", draft_count=0, response_requirements=("Ask for the consultant's email address without inventing an address.",)),
        draft("The consultant is alice@example.com. Please create the draft now."))),
    Case("read_specific", "search", (T("Find Alice's Cedar launch email and tell me its reference code. Alice is alice@example.com.", draft_count=0, response_requirements=("Report CEDAR-42 as the Cedar launch reference.",)),), CEDAR),
    Case("read_distractors", "search", (T("What is the final Cedar launch time in Alice's emails (alice@example.com)? Ignore Birch and sales offers.", draft_count=0, response_requirements=("Report Friday at 14:00 as the final Cedar time, not Thursday or Tuesday.",)),), CEDAR),
    Case("read_thread_facts", "search", (T("Read Alice's Cedar launch thread, alice@example.com. What was the earlier proposal and what is the final plan?", draft_count=0, response_requirements=("Explain that Thursday at 10:00 was the earlier proposal and Friday at 14:00 is the final plan.",)),), CEDAR),
    Case("no_match", "search", (T("Find my invoice from lunar-robotics@example.com. If no email matches, say so; don't make one up.", draft_count=0, response_requirements=("Report no matching invoice rather than inventing invoice facts.",)),), CEDAR),
    Case("search_compose", "search_action", (draft("Find Alice's final Cedar launch details (alice@example.com). Draft an update to bob@example.com, subject 'Cedar briefing', including the reference and final time. Do not send.", E(BOB, "Cedar briefing", ("CEDAR-42", "Friday", "14:00"))),), CEDAR),
...
DEVELOPMENT = tuple(replace(c, search_required=bool(c.mail), evidence_ids=(
    () if c.name == "no_match" else
    ("cedar-old", "cedar-current") if c.name == "read_thread_facts" else
    ("birch",) if c.name == "reply_correct_thread" else ("cedar-current",)
)) if c.mail else c for c in DEVELOPMENT)

# Explicit, fixed variations. No model output is used to generate fixtures or expectations.
_VARIANTS = (
    ("draft_only", "short_compose", "Please prepare, but don't send: recipient alice@example.com; subject 'Cedar update'; body exactly 'Launch Friday'."),
    ("approve_draft", "approval_wording", "Write a draft for alice@example.com with subject 'Cedar update' and body exactly 'Launch Friday'. I'll approve it later."),
    ("content_restrictions", "confidential_budget", "Draft to alice@example.com with subject 'Cedar update'. The body should state only that launch is Friday and the reference is CEDAR-42. Do not include the confidential $800 budget."),
    ("read_specific", "reference_wording", "Look up the reference identifier in Alice's Cedar launch correspondence. Her address is alice@example.com."),
    ("read_distractors", "time_wording", "Check alice@example.com's final Cedar schedule. What day and time did they settle on?"),
    ("no_match", "absent_receipt", "Look for a lunar-robotics@example.com invoice and report whether one exists. Don't guess."),
    ("search_compose", "briefing_wording", "Use alice@example.com's final Cedar email to draft to bob@example.com with subject 'Cedar briefing'. Include the reference, day and time. Show me; don't send."),
    ("withhold_approval", "hold_wording", "Draft to alice@example.com with subject 'Cedar update' and body exactly 'Launch Friday'. Await my approval."),
    ("reply_correct_thread", "thread_wording", "Locate Birch, not Cedar, from alice@example.com. Show a proposed reply with body exactly 'Birch accepted', before you send anything."),
    ("send_failure", "send_error_variant", "Please draft to alice@example.com, subject 'Cedar update', body exactly 'Launch Friday'. Let me review it."),
    ("revise_then_approve", "revision_wording", "Prepare a draft for alice@example.com with subject 'Cedar update' and body exactly 'Launch Friday', for my review."),
    ("select_pending_draft", "selection_wording", "Prepare two unsent drafts for review: Cedar update to alice@example.com saying Launch Friday; Birch update to bob@example.com saying Launch Tuesday."),
)
